...
namespace InfoTec.Bancos //alterar o namespace para o nome do seu projeto
{
    public static class ConexaoSQLite
    {
        ...
        public static void CriarTabelas()
        {
            try
            {
                ...
                using (var cmd = DbConnection().CreateCommand())
                {
                    cmd.CommandText = "CREATE TABLE IF NOT EXISTS Usuario(Id int,Login VarChar(100), Senha VarChar(100))";
                    cmd.ExecuteNonQuery();
                }
            }
            catch (Exception ex)
        ...
        public static void EditarCliente(Cliente obj) //Subtituir Objetc pelo nome da classe pessoa que foi criado
        {
            try
            {
                using (var cmd = DbConnection().CreateCommand())
                {
                    cmd.CommandText = "Update Cliente SET id = @id, Nome = @nome, CPF = @cpf, Telefone = @telefone, Email = @email where id = @id";
                    //incluir os atributos do objeto que foi passado no parametros. Ex.: obj.Id
                    cmd.Parameters.AddWithValue("@id", obj.Id);
                    cmd.Parameters.AddWithValue("@nome", obj.Nome);
                    cmd.Parameters.AddWithValue("@cpf", obj.Cpf);
        ...
        public static void CadastrarProdutos(Produtos obj)
        {
            try
            {
                using (var cmd = DbConnection().CreateCommand())
        ...
        public static void EditarProduto(Produtos obj)
        {
            try
            {
                using (var cmd = DbConnection().CreateCommand())
        ...
        //CADASTRO VENDAS
        public static void CadastrarVenda(Vendas obj)
        {
            try
            {
                using (var cmd = DbConnection().CreateCommand())
                {
                    ...
                    //incluir os atributos do objeto que foi passado no parametros. Ex.: obj.Id
                    cmd.Parameters.AddWithValue("@id", obj.IdVenda);
        ...
        public static void exibirProdutosVendas(int id)
        {
            try
            {
                using (var cmd = DbConnection().CreateCommand())
                {
                    cmd.CommandText = "SELECT * FROM ProdutosVendas WHERE IdVenda = @idvenda";
